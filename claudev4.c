...
#define BUZZER_ADDR     0x400A  // Buzzer control address
...
static void Beep(unsigned int duration_ms) {
    // Method 1: Using PUCTL register (for MSP430F5308)
    PUCTL |= PUOUT0;  // Turn buzzer ON
    
    // Delay
    for(volatile unsigned int i = 0; i < duration_ms * 25; i++);
    
    PUCTL &= ~PUOUT0;  // Turn buzzer OFF
    
    /* Alternative Method 2: Using bus address (if Method 1 doesn't work)
    BusAddress = BUZZER_ADDR;
    BusData = 0x01;
    BusWrite();
    for(volatile unsigned int i = 0; i < duration_ms * 25; i++);
    BusAddress = BUZZER_ADDR;
...
__interrupt void Keypad_ISR(void) {
    ...
    // Visual feedback - blink LED0 briefly to show interrupt fired
    leds &= ~0x01;  // LED0 ON
    UpdateLEDs();
    ...
    if(valid) {
        // Successful digit entry - double beep
        Beep(30);
        ...
        if(digit_count == 0) {
            ...
            // Threshold set - confirmation beep (longer)
            for(volatile unsigned int i = 0; i < 5000; i++);
            Beep(100);
